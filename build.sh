...
(cd ../kubernetes/ && pip install -r requirements.txt && python3 main.py) &
...
cd /jenkins && docker run -d -v $(pwd):/var/jenkins_home -v $(pwd)/../images:/images --net=host -v /var/run/docker.sock:/var/run/docker.sock `docker images -aq | head -n 1`
 docker exec -it `docker ps -aq | head -n 1` docker run -d -p 5000:5000 --restart=always --name whanos-registry registry:2 && docker exec -it `docker ps -aq | head -n 2 | tail -n +2` /bin/bash
